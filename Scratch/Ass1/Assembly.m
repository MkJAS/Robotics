...
base1 = [0,0.52,0];     %Input desired bass positions here
base2 = [0.4,-0.6,0];
...
qr2 = zeros(1,7);         %Move UR5 so its starts at centre of rail
qr2(1,1) = -0.4;
qr2(1,6) = -(pi/2);
robot2.model.animate(qr2) %

qr1 = zeros(1,7);         %Move UR3 to centre of rail
qr1(1,1) = -0.4;
robot1.model.animate(qr1) 

hold on
PlaceObject('cage.ply')         %Place cage around robots
...
%Get original robot positions for later
...
XYplane2(:,1) = XYZ2;
index = 2;
for j=-0.8:0.4:0        %Move robots along rail and then plot radius because I was too tired of this assignment at this point to do it properly
    qr1(1,1) = j;
    qr2(1,1) = j;
    for i=1:5:370
...
bricknum = 1;
for i=1:9   %Distance of each brick to UR3 base
    distances(i,:) = [(((base1(1) - brickposes{1,i}(1,1))^2 + (base1(2) - brickposes{1,i}(1,2))^2)^0.5) bricknum];
    bricknum = bricknum + 1;
end

distances = sortrows(distances,1);  %sort ascending
UR3bricks = zeros(3,1);
count = 1;
...
UR3drops = [droppoints(1,:);droppoints(2,:);droppoints(4,:);droppoints(7,:)];   %Drop off order for UR3
UR5drops = [droppoints(3,:);droppoints(6,:);droppoints(5,:);droppoints(8,:);droppoints(9,:)];   %Drop off order for UR5
...
pause(1);   %Remove plots from figure
...
%%
 if abs(m) == inf           %Get angle of line that brick wall is built along
    brickrot = trotz(pi/2);
    elseif m == 0
        brickrot = 1;
    else
 ...
 one1 = cell(4,1);  %Garbage method to store all transforms and q matrices
 one2 = cell(4,1);
 two1 = cell(4,1);
 two2 = cell(4,1);
 three1 = cell(4,1);
...
for i=1:4
    ...
    %UR$bricks(i,1) is the index corresponding to the brick which is being
    %moved 
